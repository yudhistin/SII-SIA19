-- ============================================================
-- DS2: PostgreSQL 14 - Schema for Airbnb Rentals + Hosts (Agents)
-- SII-SIA19: Integrated Real Estate Market Analysis System
--
-- Dataset: Inside Airbnb - Amsterdam, detailed listings
-- Source : https://insideairbnb.com/get-the-data/
-- File   : listings.csv.gz  (Amsterdam quarterly snapshot)
-- Volume : ~9,000 active listings / quarter
-- Role   : RENTAL listings + host (agent) information
-- ============================================================

-- Pas 1: Copy CSV into the Postgres container
-- gunzip listings.csv.gz
-- docker cp listings.csv postgresql-container:/tmp/listings_amsterdam.csv

-- Pas 2: Schema and tables
CREATE SCHEMA IF NOT EXISTS rentals;

-- Hosts (acting as Agents in our domain model)
...
-- Pas 3: Staging table that mirrors the raw Inside Airbnb CSV column order
-- Inside Airbnb listings.csv has 75 columns; we stage all of them as TEXT,
-- then project only the typed subset we care about.
...
-- Pas 4: Load CSV (run from psql)
-- \COPY rentals.stg_listing FROM '/tmp/listings_amsterdam.csv' CSV HEADER DELIMITER ',' QUOTE '"' ESCAPE '"';
...
-- Pas 7: Verification queries
SELECT 'hosts' AS table_name, COUNT(*) FROM rentals.host
UNION ALL
...
SELECT neighbourhood_cleansed, room_type, COUNT(*) NR,
       ...
       ROUND(AVG(review_scores_rating)::numeric, 2) AVG_RATING
...
SELECT host_is_superhost, COUNT(*) NR_HOSTS,
       ROUND(AVG(host_listings_count)::numeric, 1) AVG_LISTINGS_PER_HOST
FROM rentals.host
